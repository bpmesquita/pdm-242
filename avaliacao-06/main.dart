...
class CarrinhoProvider extends ChangeNotifier {
  ...
  double get total =>
      _produtosSelecionados.fold(0, (soma, produto) => soma + produto.preco);
...
class ProdutosScreen extends StatelessWidget {
  final List<Produto> produtos = [
    Produto(nome: 'Pão', preco: 10.0),
    Produto(nome: 'Sabão', preco: 15.0),
    Produto(nome: 'Chiclete', preco: 8.0),
    Produto(nome: 'Café', preco: 20.0),
  ];
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: carrinho.produtos.isEmpty
            ? null
            : () => Navigator.push(
                context, MaterialPageRoute(builder: (_) => ClienteScreen())),
            ...
            ElevatedButton(
              onPressed: carrinho.clienteNome.isEmpty
                  ...
                  : () => Navigator.push(context,
                      MaterialPageRoute(builder: (_) => PedidoScreen())),
...
class PedidoScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String itens = carrinho.produtos
        .map((produto) =>
            '${produto.nome} - \$${produto.preco.toStringAsFixed(2)}')
        .join(' + ');
              ...
              child: ElevatedButton(
                onPressed: () {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                          '$itens = Total \$${carrinho.total.toStringAsFixed(2)}'),
